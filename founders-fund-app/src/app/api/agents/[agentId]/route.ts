...
export async function GET(
  ...
  try {
    ...
    const totalPnl = latestPerf
      ? Number(latestPerf.realizedPnl) + Number(latestPerf.unrealizedPnl)
      : 0;
  ...
  try {
    ...
    if (status) {
      ...
      if (status === 'ACTIVE' || status === 'PAUSED' || status === 'CLOSED') {
        ...
        return NextResponse.json(
          { error: 'Invalid status. Use: ACTIVE, PAUSED, or CLOSED' },
          { status: 400 },
        );
      }
    }
    if (strategy) {
      updateData.strategy = strategy as Record<string, unknown>;
    }
    ...
    const prismaUpdateData: Prisma.AgentUpdateInput = {};
    if (updateData.status) {
      prismaUpdateData.status = updateData.status;
    }
    if (updateData.strategy) {
      prismaUpdateData.strategy = updateData.strategy as unknown as Prisma.InputJsonValue;
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (agent.status !== AgentStatus.CLOSED) {
      return NextResponse.json({ error: 'Agent must be closed before deletion' }, { status: 400 });
    }
